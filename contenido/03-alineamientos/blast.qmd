...
8](sesion08-practica.qmd). Arma un pipeline completo con BLAST+ local
sobre la vía WNT de *Deroceras laeve*, en el servidor. El concepto de
cómputo del día es **parsear salida tabular** con `cut`, `sort` y
`awk`.

1.  **Montar la base.** El proteoma anotado de la babosa (43,905
    proteínas, una por gen) con `makeblastdb -dbtype prot
    -parse_seqids`. Desde BLAST+ 2.10 el default es la base versión 5.
2.  **Buscar en local.** 72 proteínas de referencia de 18 genes de la
    vía WNT (humano y tres moluscos) con `blastp -outfmt "6 std qcovs"`.
    En el servidor, con las bases montadas, siempre local: control de
    versión, `-num_threads`, sin límites de servidor. `-remote` sólo
    como demostración.
3.  **Parsear.** El formato `-outfmt 6` son doce columnas separadas por
    ...
    de `cut`/`sort`/`awk`: mejor hit por consulta, censo de la familia
    LGR con E-value **y** cobertura, ubicación de los candidatos en el
    genoma, y el script del entregable, que reporta los top-5 hits de
    cualquier FASTA.
4.  **Alinear la familia.** Los candidatos recuperados de la base con
    `blastdbcmd` se alinean con MAFFT, que es el puente al [capítulo de
    alineamiento múltiple](alineamiento-multiple.qmd). Un bonus de mejor
    hit recíproco anticipa la sesión 10.
